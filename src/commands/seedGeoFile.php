...
class seedGeoFile extends Command
{
    ...
    public function handle()
    {
        ...
        $country = null;
        $seed = function ($path) use ($start) {
            $this->geoItems = new geoCollection();
            $country = explode('/',$path);
            $country = $country[count($country)-1];
            $fileName = $path."/{$country}.txt";
...
//                $alternames=json_encode(['es'=>$name,'en'=>$name]);
        ...
        $countries = glob(storage_path("geo/{$country}/*"), GLOB_ONLYDIR);
        if(count($countries) > 0){
            array_map($seed, $countries);
            $this->info("All countries in region were saved</info>");

            return true;
        }

        $country = $this->argument('country') ? strtoupper($this->argument('country')) : 'allCountries';
        $seed(storage_path("geo/{$country}"));
    }
}
